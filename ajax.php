...
function xtra_delete_backup() {
	...
	if ( is_array($ids) && count($ids) ) {
		...
		$results = array( 
			...
			'id'=> $id, 
			'fromi'=> $i, 
			'message' => sprintf( esc_html__( $xsure.'Delete Backup Files (Bulk): OK: %s, NO-FILE: %s, ERROR: %s', 'xtra' ) ,$ok,$nf,$er )
		);
		//if ($xtra_ajax_sure) 
...
/*		if ( ! is_writable( $backupdir."/".$reladir ) ) {
			die( json_encode( array( 'success' => false, 'message' => sprintf( esc_html__( '%s is not writable', 'xtra' ), $backupdir."/".$reladir ) ) ) );
		}
		*/
...
function xtra_restore_image() {
	...
	if ( is_array($ids) && count($ids) ) {
		...
		$results = array( 
			...
			'id'=> $id, 
			'fromi'=> $i, 
			'message' => sprintf( esc_html__( $xsure.'Restore (Bulk): OK: %s, NO-FILE: %s, ERROR: %s', 'xtra' ) ,$ok,$nf,$er )
		);
	}
...
function xtra_restore_image_do($source_path) {
	...
	$reladir = str_replace(array($upload_dir."/","/".$filename),"",$source_path);
		...
		if ( ! is_writable( dirname($source_path) ) ) {
			die( json_encode( array( 'success' => false, 'message' => sprintf( esc_html__( '%s is not writable', 'xtra' ), dirname($source_path) ) ) ) );
		}
		if ( !file_exists($backupdir."/".$reladir."/".$filename) )
			return array(
				'restoreOK'		=>	"NO-FILE",
...
function xtra_compress_image()
{
	...
	if (!$compQ || $compQ>100 || $compQ<1) $compQ = get_option('xtra_custom_jpeg_quality_num',82);
	...
	if ( is_array($ids) && count($ids) ) {
		...
		foreach($ids as $id) {
			...
			// --start-proc--
			if ( ! is_writable( $id ) ) {
				die( json_encode( array( 'success' => false, 'message' => sprintf( esc_html__( '%s is not writable', 'xtra' ), $id ) ) ) );
			}
			...
			if ( $mem['imgmem'] >= $mem['avamem'] )
				die( json_encode( array( 'success' => false, 'message' => "Too large image ($imgW x $imgH = ".$mem['imgmemMB']."M) - not enough memory (".$mem['maxmemMB']."M allowed - ".$mem['curmemMB']."M used = ".$mem['avamemMB']."M available)" ) ) );
			...
			elseif ($xMethod==2) {
				...
				$res = array('imageOK'=>'ERROR: Invalid Method!!!');
				$method = "";
			}
			// --end-proc--
			if (InStr($res['imageOK'],"OK")) $ok++;
			if (InStr($res['imageOK'],"ERROR")) $er++;
			if (InStr($res['imageOK'],"RESIZED")) $rs++;
			if ($i >= $fromi+$maxi-1) break;
		}
		$results = array( 
			'success'=>true, 
			'id'=> $id, 
			'fromi'=> $i, 
			'message' => sprintf( esc_html__( $xsure.'Compress (Bulk): OK: %s, RESIZED: %s, ERROR: %s', 'xtra' ) ,$ok,$rs,$er )
		);
	}
...
function xtra_verify_permission()
{
	if ( ! current_user_can( 'manage_options' ) ) {
		die( json_encode( array( 'success' => false, 'message' => esc_html__( 'Administrator permission is required', 'xtra' ) ) ) );
	}
	if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'xtra_ajax_nonce' ) ) {
		die( json_encode( array( 'success' => false, 'message' => esc_html__( 'Access token has expired, please reload the page.', 'xtra' ) ) ) );
	}
}
